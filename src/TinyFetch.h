...
/**
 * @brief A lightweight HTTP client wrapper using the Arduino HTTPClient
 * library.
 *
 * TinyFetch simplifies making HTTP requests (GET, POST, PUT, DELETE) to a base
 * URL.
 */
class TinyFetch {
public:
   ...
   * Initializes TinyFetch and attempts to read the base URL from persistent
   * storage.
   */
  TinyFetch();
  ...
  /**
   * @brief Retrieves the current base URL.
   *
   ...
   * @param contentType The content type of the request body (default is
   * "application/json").
   * @return An HttpResponse containing the status code and payload.
   */
   ...
   * @return An HttpResponse containing the status code and payload.
   */
  HttpResponse put(const String &path, const String &body,
                   const String &contentType = "application/json");
